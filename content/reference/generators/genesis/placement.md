...
**Genesis Placement** — status **draft**, version **v1**.

The placement generator decides **how many systems the cluster holds and which
hexes they occupy**. It runs first, and hands the placed systems to the
[system-contents generator]({{< relref "/reference/generators/genesis/system-contents.md" >}}).

It does not decide anything about what is *in* a system.
...
- **Number of systems** (`N`) is how many systems the finished cluster contains.
- **Stellar density** (`D`) is how large an area those systems are spread across,
  from `extremely dense` (a small, tightly packed map) to `very sparse` (a large,
  loosely packed one). It governs the *average* distance between systems.
- **Minimum spacing** (`S`) is the smallest distance, in hexes, allowed between any
  two systems — a hard floor applied when they are placed. It has no maximum, and
  unlike the others it is **not** derived from the density.

## From the settings to a map

The three settings feed two values the placement algorithm consumes:
...
`N` and `D` set how large the map is and how far apart systems sit *on average*;
`S` sets the *guaranteed minimum* gap between any two systems, independently of the
density. See [the minimum-spacing knob](#the-minimum-spacing-knob).

All distances are hex distances, measured with the metric defined under
[Cluster]({{< relref "/reference/cluster.md" >}}#measuring-distance).
...
- **Average nearest neighbor** — the expected distance from a system to its
  ...
  while exploring. (Measured here at `S = 1`; the default `S = 2` nudges each
  figure up ~0.25 — see [the minimum-spacing knob](#the-minimum-spacing-knob).)
...
**Minimum spacing `S` is an independent setting** — an integer of at least `1`,
...
`S` hexes from every system already placed.

Density and spacing both push systems apart, but act on different parts of the
distribution. Density (radius) raises the **average** distance between systems; `S`
raises the **guaranteed minimum**. Measured per-system nearest-neighbor
distribution at `N = 100`:
...
Both bottom rows reach roughly the same mean (~4.7) by different means. Raising the
**density** to `very sparse` still leaves about one system in five with a neighbor
two hexes away or closer, and inflates the map ~40%. Raising **`S`** to `3` removes
the close neighbors entirely (0% within two hexes) while the map stays the same
size. The three concerns separate cleanly:
...
fallback: the generator does not relax `S`, grow the radius, or build a partial map.
...
Once the first real game is generated, these radii become part of that game's
reproducibility contract: the same `N` and `D` must always yield the same `R`, so
the same seeds reproduce the same map. Changing the radius formula would change
every existing map, so it is not changed — a retuned placement generator is
published as a **new version**, and games running v1 keep v1's maps. The worked
examples above are effectively frozen the day a game is first persisted.
...
chosen density and spacing. The GM must change the settings and generate again.

## Reproducibility

The shuffle and every draw come from the cluster's master seeds, which are derived
from the game's. The same seeds and the same settings always reproduce the same
placement — the same systems in the same hexes. See
[Determinism]({{< relref "/reference/determinism.md" >}}).

## See also

- [Cluster]({{< relref "/reference/cluster.md" >}}) — what a cluster is, and the distance metric
- [Genesis]({{< relref "/reference/generators/genesis" >}}) — the family this generator belongs to
- [System contents]({{< relref "/reference/generators/genesis/system-contents.md" >}}) — the stage that runs next
